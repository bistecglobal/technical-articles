...
- *Closing the Loop: A Schedule Run History and Outcome Tracker for Autonomous Agents* — MCD's P217 records every scheduled agent run with its outcome, turning fire-and-forget cron-style schedules into an auditable history with success/failure tracking [commit `0670b47` P217] ✅ Published as *Did the Schedule Actually Fire?* ([bistecglobal/blog#57](https://github.com/bistecglobal/blog/issues/57))
- *Drawing the Family Tree: Visualizing the Agent Spawn Hierarchy* — MCD's P229 builds an agent spawn tree showing which sessions spawned which subagents, making multi-agent delegation chains legible [commit `102e8db` P229 #226] ✅ Published ([bistecglobal/blog#58](https://github.com/bistecglobal/blog/issues/58))
- *How Much Runway Is Left? Per-Agent Context Exhaustion Forecasting* — MCD's P219 context runway projects how many turns each agent has before it hits its context limit, surfacing imminent resets before they happen [commit `099ca54` P219] ✅ Published ([bistecglobal/blog#54](https://github.com/bistecglobal/blog/issues/54))
- *MTTR for AI Subprocesses: Measuring Circuit-Breaker Recovery Across a Fleet* — MCD's P227 circuit breaker timeline + P233 MTTR table quantify how fast crashing Claude subprocesses recover, applying classic SRE reliability metrics to AI agents [commits `aba4f72` P227, `bbba5b3` P233] ✅ Published ([bistecglobal/blog#53](https://github.com/bistecglobal/blog/issues/53))
- *Forecasting Autonomous Throughput: A Backlog Completion Projection* — MCD's P234 backlog forecast extrapolates recent completion velocity to project when an autonomous agent will clear its BACKLOG.md [commit `bbba5b3` P234] ✅ Published as *When Will the Agent Finish?* ([bistecglobal/blog#56](https://github.com/bistecglobal/blog/issues/56))
...
- *The Sankey That Shows How Agents Move: Visualizing Fleet State Transitions* — MCD's P237 renders a state-transition Sankey (idle→active→stalled→autonomous) so operators see how their fleet flows between lifecycle states, paired with P236's per-tool error-rate monitor [commit `181baff` P236+P237] ✅ Published ([bistecglobal/blog#59](https://github.com/bistecglobal/blog/issues/59))
...
- *Digital Twins for AI Agents: Finding the Project Most Like This One* — MCD's P258 Project Twin Analysis surfaces, for any agent, its nearest-neighbour project across activity/memory/convergence dimensions — a similarity lookup for triage and pattern transfer [commit `0600993` P258] ✅ Published ([bistecglobal/blog#60](https://github.com/bistecglobal/blog/issues/60))
- *How Long Until It Answers? A Response-Latency Distribution for an AI Fleet* — MCD's P256 computes per-project response-latency histograms from transcript timestamps, exposing slow-responding agents the averages hide [commit `bc66776` P256] ✅ Published as *The Average Is Lying to You* ([bistecglobal/blog#64](https://github.com/bistecglobal/blog/issues/64))
...
- *The Operator Inbox: A Unified Action Queue for Steering an Agent Fleet* — MCD's P248 Operator Inbox aggregates everything needing a human decision across all projects into one prioritized queue, alongside P249's per-session memory timeline [commit `7d7ccec` P248+P249] ✅ Published as *Eleven Dashboards, One Question* ([bistecglobal/blog#65](https://github.com/bistecglobal/blog/issues/65))
...
- *One Heatmap, Every Tool: Cross-Project Tool Usage at a Glance* — MCD's P270 renders a project × tool heatmap of call frequency, exposing which agents lean on which tools and where usage is anomalous [commit `c2a4a2e` P270] ✅ Published ([bistecglobal/blog#66](https://github.com/bistecglobal/blog/issues/66))
...
- *Closing the Memory Loop: A Recovery Feedback Cycle for Agents That Forget* — MCD's P293 Memory Recovery Feedback Loop flags orphaned memory files (zero inbound/outbound `[[links]]`) via a link-graph scan and tracks each across a lifecycle (flagged → relinked/deleted/ignored) without ever auto-deleting [commit `4dadf07` P293] ✅ Published ([bistecglobal/blog#69](https://github.com/bistecglobal/blog/issues/69)) — note: shipped feature is orphan-file lifecycle tracking, not prompt injection as originally described
- *Catching a Tool Going Bad: Spike Detection on Per-Tool Failure Rates* — MCD's P292 Tool Failure Spike Detector watches per-tool error rates across the fleet and flags sudden spikes (a failing MCP server, a broken Bash path) before they cascade, paired with P291's alert-delivery audit log proving every alert was actually delivered [commit `c442538` P291+P292] ✅ Published as *When Three Agents Fail at Once* ([bistecglobal/blog#68](https://github.com/bistecglobal/blog/issues/68))
- *Why Did the Watchdog Fire? A Heatmap of Agent Kill Patterns* — MCD's P289 Watchdog Kill Pattern Heatmap cross-tabs which projects get killed by the stall watchdog at which times, surfacing systemic kill patterns vs one-off stalls, alongside P290's proposal-implementation-lag tracker [commit `0278ed0` P289+P290] ✅ Published — P289 as ([bistecglobal/blog#71](https://github.com/bistecglobal/blog/issues/71)) "The Watchdog's Logbook"; P290 still open
- *Multi-Tenant by Construction: Postgres Row-Level Security for an AI Agent Platform* — agent-nexus moved to a pure-tenant model enforced by Postgres RLS, pushing tenant isolation into the database itself rather than trusting every query to filter correctly — a strong, generalizable security-architecture case study [PR 237, commit `1596b7a`] ✅ Published as *One Forgotten WHERE Clause* ([bistecglobal/blog#67](https://github.com/bistecglobal/blog/issues/67))
- *Hot-Reloading the Brain: Live StepGraph Plugin Reloads Without Restarting Agents* — agent-nexus's Plugin StepGraph hot-reload swaps agent workflow plugins at runtime without tearing down the process, keeping long-running agents online through logic updates [PR 244, commit `ade2161`] ✅ Published as *The Hot-Reload We Shipped Turned Off* ([bistecglobal/blog#70](https://github.com/bistecglobal/blog/issues/70)) — note: shipped default is OFF in prod (mid-session structural-swap risk); dev-loop opt-in
