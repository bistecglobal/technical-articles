# Drawing the Family Tree: Visualizing the Agent Spawn Hierarchy

*When one AI agent spawns another, the delegation structure vanishes into a flat transcript. Here's how we drew it back out.*
...
First, **structure is a first-class observability signal, not a derived one.** We already had transcript replay, token accounting, and tool-call stats. None of them surfaced delegation shape, because shape isn't a number — it's a topology. Reconstructing it took a deliberate two-pass parse; it would never have fallen out of a metrics aggregation. First-level delegation is exact and the deeper inference is clearly bounded — and naming that limit in the code is what keeps the view trustworthy. An observability tool that quietly guesses is worse than one that shows you exactly how much it knows.
