# The Note Nobody Links To: Tracking Orphaned Memory Across an AI Agent Fleet

*Age tells you how long a memory has sat. Orphan-status tells you whether it's wired into anything at all — and that turned out to be the more useful signal.*
...
If a previously-flagged note has dropped off the current orphan list, something changed it for the better — so the endpoint auto-resolves it. It checks whether the file still exists on disk: if it does, the note must have been linked back into the graph, so the resolution is `relinked`. If the file is gone, the resolution is `deleted`. The only manual action is `ignored`, set through a `POST` when an operator decides an isolated note is fine as-is.
