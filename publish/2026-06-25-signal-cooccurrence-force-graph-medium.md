# Which Alerts Travel Together? A Co-occurrence Graph for an AI Agent Fleet

*Flat alert streams hide the one thing that matters most: which problems are really the same problem.*
...
The whole feature is one API route and one page. The route turns the event log into a weighted graph.
...
- **Bucket = project-day.** Co-occurrence has to mean "on the same project, in the same window," not "somewhere in the fleet this month." Two signals that fire on different projects are not related.
- **Worst-severity wins per node.** The node carries the worst severity it ever reached, so the color coding answers "how bad does this get," not "how bad was it last time."
- **Signals are sorted before pairing.** Sorting means `a|b` and `b|a` collapse to one edge key. No double counting, no directionality we don't have.
- **Healthy is not a signal.** Anything with an `ok` severity is dropped before bucketing. The graph is a map of problems, not a map of everything.

## Degrading gracefully when there's no history

A graph built from history is empty on a fresh install. Rather than show a blank canvas, the route degrades to a **live snapshot**: when the event table returns nothing, it calls the same attention engine the Brief uses and buckets the current findings by project.
...
The page badges which mode produced the graph, so an operator is never misled about whether they're looking at a 30-day pattern or a single instant.

## Letting the layout do the explaining

The front end reuses the force simulation that already powers Mission Control's memory graph. Nodes are sized by total firings (square-root scaled). Edges pull harder and draw thicker the more two signals co-occur:
...
The payoff: tightly-correlated signals physically clump. A dense, short-edged cluster *is* a correlated failure mode. Hovering a node lists the projects firing it, each a deep-link straight to that project's Fleet Brief — so the graph is the front door to triage, not a dead end.

## What it changed

The view costs almost nothing to run — a single read of an existing table, a 60-second refresh — because every input already existed. That's the quiet lesson: the most useful observability often isn't new instrumentation, it's a new *projection* of telemetry you're already collecting.

Triage used to start with a list and a guess. It now starts with a shape. When two signals are bound by a thick edge, we stop treating them as two incidents and start hunting for the one cause underneath.
