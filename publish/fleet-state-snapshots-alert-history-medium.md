...
Because the SSE broadcaster already runs on a 5-second interval and already computed the stalls, wiring in persistence cost exactly one function call per stall event. No new polling loop, no separate process.
