...
*How we used Jaccard similarity over agent memory files to discover which AI projects in our fleet share the same operational knowledge — and what to do about it.*
...
Hovering any non-diagonal cell opens a tooltip with the two project slugs, the similarity percentage, and the top-5 shared keywords ordered by combined frequency. That last piece is what makes the heatmap actionable rather than just interesting. Knowing that two projects share a similarity score is data; knowing that their shared keywords are `authentication`, `budget`, `session`, `token`, and `agent` is insight.
