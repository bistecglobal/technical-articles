...
*How we replaced four 30-second polling loops with a single SSE connection and cut dashboard update latency from 30 seconds to 5.*
...
## The Problem: Polling Requests Per Minute
...
The Next.js route handler returns a `ReadableStream` with `Content-Type: text/event-stream`. Each connecting client is registered with `addClient()` and deregistered via the `cancel()` hook when the connection closes. A keepalive comment-line fires every 5 seconds to prevent proxy timeouts:
