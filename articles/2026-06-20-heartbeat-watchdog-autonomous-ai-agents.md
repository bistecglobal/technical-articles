...
3. **Process-crashed** — the tmux session exits (OOM, network drop, bad restart) and the process disappears.
...
The `staleAfterMinutes` threshold (default 60, configurable per-project via `heartbeat.staleAfterMinutes` in the channels config) controls how old an anomaly must be before it is treated as a real stall. A `tool-incomplete` from 2 minutes ago is likely a transient race; one from 90 minutes ago is a dead channel.
...
The pool tracks the last 5 turn durations (`MAX_TURN_HISTORY = 5`). The effective threshold becomes `max(5min, ceil(longest_recent_turn × 1.5))`, capped at 30 minutes. A channel that routinely completes 12-minute turns gets an 18-minute watchdog window; one that always finishes in under 90 seconds stays at 5 minutes.
